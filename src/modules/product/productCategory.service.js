...
export const getCategories = async ({ userId }) => {
  try {
    ...
    throw new Error('Error al obtener las categorías del usuario');
  }
};
...
export const createCategory = async ({ userId, categoryData }) => {
  try {
    ...
    if (color && !/^#[0-9A-F]{6}$/i.test(color)) {
      throw new Error('El color debe estar en formato hexadecimal válido (#RRGGBB)');
    }
...
export const deleteCategory = async ({ userId, categoryId }) => {
  try {
    ...
    if (productsCount > 0) {
      throw new Error(`No se puede eliminar la categoría porque tiene ${productsCount} producto(s) asociado(s). Primero mueva los productos a otra categoría o elimínelos.`);
    }
