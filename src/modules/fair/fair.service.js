...
export const getFairs = async ({ userId }) => {
  try {
    ...
    throw new Error('Error al obtener las ferias del usuario');
  }
}
...
export const createFair = async ({ userId, fairData }) => {
  try {
    ...
    if (!name || !location || !start_date || !end_date) {
      throw new Error('Nombre, ubicación, fecha de inicio y fecha de fin son requeridos');
    }
    ...
    console.error('Error in createFair service:', error);
    throw error;
  }
}
...
export const updateFair = async ({ userId, fairId, updateData }) => {
  try {
    ...
    console.error('Error in updateFair service:', error);
    throw error;
  }
}
...
export const deleteFair = async ({ userId, fairId }) => {
  try {
    ...
    console.error('Error in deleteFair service:', error);
    throw error;
  }
}
...
export const getFairStats = async ({ userId }) => {
  try {
    ...
    throw new Error('Error al obtener estadísticas de ferias');
  }
}
...
export const updateFairExpenses = async ({ userId, fairId, expenses }) => {
  try {
    ...
    if (expenses === undefined || expenses === null) {
      throw new Error('El campo expenses es requerido');
    }

    // Validar que expenses sea un número válido
    if (isNaN(parseFloat(expenses)) || parseFloat(expenses) < 0) {
      throw new Error('Los gastos deben ser un número válido mayor o igual a 0');
    }
    ...
    console.error('Error in updateFairExpenses service:', error);
    throw error;
  }
}
